...
# dexp pulls cupy (GPU); it's optional at IMPORT so this module loads in
# dexp-less envs (e.g. cp_dino_env). The names below are used only inside the
# GPU registration/solver functions (register_translation_gpu, offset,
# optimal_positions), which run where dexp is present.
try:
